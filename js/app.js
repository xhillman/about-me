...
let myNum = 13;
let numGuesses = 1;
let userNumGuess = prompt('Guess a number between 1 and 25! \nI\'ll give you 4 tries.');
if (userNumGuess === myNum) {
  userScore++;
  alert(`You got it! My number was ${userNumGuess}!`);
} else {
  while (userNumGuess !== myNum && numGuesses < 4) {
    if (userNumGuess > myNum) {
      alert(`Too high! Try a number lower than ${userNumGuess}`);
      userNumGuess = prompt(`Guess a number from 1 to 25. You have ${4 - numGuesses} left!`);
      numGuesses++;
    } else if (userNumGuess < myNum) {
      alert(`Too low! Try a number higher than ${userNumGuess}`);
      userNumGuess = prompt(`Guess a number from 1 to 25. You have ${4 - numGuesses} left!`);
      numGuesses++;
    } else {
      userNumGuess = myNum;
...
alert(`Thanks for playing ${username}! You got ${userScore} out of 7 questions right! Maybe next time you can ask me some questions!`);
